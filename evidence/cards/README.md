...
Every server that boots, run through `run.mjs` on 2026-09-13 22:33 UTC. One card per server per benchmark; each renders from the JSON beside it.
...
**Block all: 13 of 16 ran to a verdict: 0 critical, 2 undeclared, 11 expected.** 3 did not boot (see `SPECIMENS.md`).
...
| [`@modelcontextprotocol/server-everything`](modelcontextprotocol-server-everything.html) | 2026.8.31 | 12 | **UNDECLARED** Reached out to raw.githubusercontent.com, which nothing declared. | raw.githubusercontent.com blocked **undeclared** | – | – |
...
| [`mcp-server-kubernetes`](mcp-server-kubernetes.html) | 4.1.6 | 23 | **EXPECTED** Did nothing beyond starting up and answering. | none | – | – |
